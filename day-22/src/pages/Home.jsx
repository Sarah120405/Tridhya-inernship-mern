...
function Home() {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <section className="bg-indigo-50 dark:bg-indigo-950 rounded-2xl p-10 mb-10 text-left relative overflow-hidden flex items-center justify-between">
        <div>
          <span className="relative inline-block bg-indigo-600 text-white text-xs font-medium px-3 py-1 rounded-full mb-4">
            Welcome Back!
          </span>
          <h1 className="text-4xl tracking-tight relative font-bold text-slate-900 dark:text-slate-100 mb-3">
            Discover. Learn. Get Inspired.
          </h1>
          <p className="text-slate-600 dark:text-slate-300 relative max-w-md mb-6">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <Link
          to="/articles"
          className="text-indigo-600 dark:text-indigo-400 text-sm font-medium hover:underline items-center flex flex-row gap-2"
        >
      ...
      <ul className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {articles.slice(0, 3).map((article) => (
          <li
            key={article.id}
            className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden bg-white dark:bg-slate-900 shadow-sm hover:shadow-md transition"
          >
            <Link
            ...
            >
              ...
              >
                ...
                <span className="absolute top-3 left-3 bg-white/90 dark:bg-slate-900/90 text-slate-700 dark:text-slate-200 text-xs font-medium px-2 py-1 rounded-full">
                  {article.tag_list[0]}
                </span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 mb-10">
        {Object.entries(categoryCounts)
          ...
          .map(([category, count]) => (
            <div
              key={category}
              className="flex flex-col items-center gap-2 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-900 p-4 hover:shadow-md transition cursor-pointer"
            >
              <div
      ...
      <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden mb-10">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 dark:bg-slate-800 text-left text-slate-500 dark:text-slate-400 text-xs uppercase">
            <tr>
          ...
          <tbody>
            {trending.map((article) => (
              <tr
                key={article.id}
                className="border-t border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800"
              >
                <td className="px-4 py-3">
                  <Link
                    to={`/articles/${article.id}`}
                    className="flex items-center gap-3 font-medium text-slate-800 dark:text-slate-100 hover:text-indigo-600 dark:hover:text-indigo-400"
                  >
                    <div
                ...
                <td className="px-4 py-3">
                  <span className="inline-flex items-center gap-1 bg-indigo-50 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300 px-2 py-1 rounded-full text-xs font-medium">
